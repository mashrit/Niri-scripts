...
int main(void)
{
	...
	while(fgets(buffer, sizeof(buffer), pipe))
	{
		...
		{
			system("notify-send file");
			system("scratchpad clear");
			continue;
		}

		// char *pgrepAns = get_cmd_output("grep cm");
		// if (!pgrepAns)
		// {
		// 	system("notify-send pgrep");
		// 	system("scratchpad clear");
		// 	continue;
		// }
		...
		printf("lastWorkIdx: %d, currWorkIdx: %d\n", lastWorkIdx, currWorkIdx);

		if (currWorkIdx != lastWorkIdx)
		{
			system("notify-send if");
			lastWorkIdx = currWorkIdx + 1;
